  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (response.data.status === "PENDING") {
           ...
           localStorage.setItem("authToken", response.data.data.token); // Save the authtoken
      // Log the token to the console
      console.log("🚀 Token retrieved successfully :", response.data.data.token);
    // Set the authToken in a cookie for sharing across domains
    document.cookie = `auth_token=${response.data.data.token}; path=/; domain=localhost; SameSite=None; Secure`;

    // Optionally, read the cookie directly here if needed
    const cookies = document.cookie;
    console.log(cookies); // Log all cookies
    ...
    console.log(authToken); // Logs the token or null if not found
